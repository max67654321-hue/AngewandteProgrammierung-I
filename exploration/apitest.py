...
def list_notes(
    ...
    if created_after:
        notes = [note for note in notes if note.created_at.isoformat() >= created_after]
    if created_before:
        notes = [note for note in notes if note.created_at.isoformat() <= created_before]
...
def list_categories(session: SessionDep) -> list[str]:
    notes = session.exec(select(Note)).all()
    return sorted(set(note.category for note in notes))
